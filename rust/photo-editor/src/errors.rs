...
impl PhotoEditorError {
    pub fn image<E>(e: E) -> Self
    where
        E: std::error::Error + Send + Sync + 'static,
    {
        PhotoEditorError::FailedToReadImage {
            source: Some(Box::new(e)),
        }
    }

    pub fn image_no_source() -> Self {
        PhotoEditorError::FailedToReadImage { source: None }
    }
...
pub enum InterpolationError {
    ...
    #[error(
        "x_pts must be strictly increasing, but found a non-increasing value at index {index}"
    )]
