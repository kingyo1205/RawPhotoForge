...
class SettingsDialog:
    ...
    def __init__(self, parent, settings_manager, main_window):
        ...
        Label(language_frame, text=main_window.tr("言語:")).pack(anchor='w', padx=10, pady=10)
        
        self.language_var = StringVar()
        language_combo = ttk.Combobox(language_frame, textvariable=self.language_var, 
                                     values=["English", "日本語"], state="readonly", width=40)
...
class RAWDevelopmentGUI:
    ...
    def init_ui(self):
        """UI初期化"""
        self.root = tk.Tk()
        self.root.title(self.tr("RAW現像ソフト"))
    ...
    def _update_mask_controls_state(self):
        ...
        is_no_mask_selected = self.mask_var.get() == self.tr("マスクなし")
        new_state = tk.NORMAL if not is_no_mask_selected else tk.DISABLED
        
        # 対象ウィジェットの状態を設定
        self.delete_mask_btn.config(state=new_state)
        self.invert_mask_btn.config(state=new_state)
        self.mask_display_check.config(state=new_state)

        # SliderWithSpinBoxの有効/無効を切り替え
        slider_widget_state = 'normal' if not is_no_mask_selected else 'disabled'
        self.mask_range_slider.slider.config(state=slider_widget_state)
        self.mask_range_slider.spinbox.config(state=slider_widget_state)

        if is_no_mask_selected:
