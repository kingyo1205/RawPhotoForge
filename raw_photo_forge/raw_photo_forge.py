...
class SettingsManager:
    ...
    def load(self):
        ...
        try:
            ...
            if RAW_EDITOR_AVAILABLE and raw_image_editor.WGPU_AVAILABLE:
                try:
                    adapters = raw_image_editor.get_wgpu_adapters()
                    ...
                    traceback.print_exc()
                    self.device_index = 0  # パースエラーやWGPUエラー
            else:
                self.device_index = 0  # WGPUが利用不可
    ...
    def __init__(self, parent, settings_manager, main_window):
        ...
        if RAW_EDITOR_AVAILABLE and raw_image_editor.WGPU_AVAILABLE:
            try:
                adapters = raw_image_editor.get_wgpu_adapters()
                if not adapters:
                    device_options.append("WGPUデバイスが見つかりません")
                    self.device_indices.append(0)
                else:
                    for i, adapter in enumerate(adapters):
                        # device_name = ctx.devices[0].name
                        device_name = f"{adapter.summary}"
                        device_options.append(device_name)
                        self.device_indices.append(i)
            except Exception:
                traceback.print_exc()
                device_options.append("WGPUデバイスの取得中にエラー")
                self.device_indices.append(0)
        else:
...
class RAWDevelopmentGUI:
    ...
    def __init__(self):
        ...
        # WGPUが利用可能かチェック
        if not (RAW_EDITOR_AVAILABLE and raw_image_editor.WGPU_AVAILABLE):
            root = tk.Tk()
            root.withdraw()
