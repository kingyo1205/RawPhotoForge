...
var Image = class _Image {
  ...
  static async createFromImageBitmap(gpuProcessor2, imageBitmap) {
    ...
    tempTexture.destroy();
    return new _Image(gpuProcessor2, finalTexture, imageBitmap.width, imageBitmap.height);
  }
  static async createFromPpm(gpuProcessor2, file) {
    const device = gpuProcessor2.getDevice();
    ...
    return new _Image(gpuProcessor2, texture, width, height);
  }
  clone() {
...
var PhotoEditor = class _PhotoEditor {
  ...
  static async create(gpuProcessor2, imageData) {
    if (imageData instanceof ImageBitmap) {
      const image = await Image.createFromImageBitmap(gpuProcessor2, imageData);
      return new _PhotoEditor(gpuProcessor2, image);
    } else {
      const image = await Image.createFromPpm(gpuProcessor2, imageData);
      return new _PhotoEditor(gpuProcessor2, image);
    }
  }
...
async function resizeBitmap(bitmap, longEdge) {
